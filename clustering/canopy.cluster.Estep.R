...
canopy.cluster.Estep = function(Tau,Mu,R,X){
    ...
    if(Tau[length(Tau)]!=0){
        pGtemp=pG
        pGtemp=pGtemp-matrix(ncol=ncol(pGtemp),nrow=nrow(pGtemp),data=apply(pGtemp,2,max),byrow = T) # prevent underflow
        pGtemp=exp(pGtemp)
        pGtemp=pGtemp/matrix(nrow=nrow(pG),ncol=ncol(pG),data=colSums(pGtemp),byrow=T)
        pG[K+1,(rank(pGtemp[K+1,],ties.method='random'))<=(ncol(pG)*(1-Tau[K+1]))]=-Inf 
    }
    pG=pG-matrix(ncol=ncol(pG),nrow=nrow(pG),data=apply(pG,2,max),byrow = T) # prevent underflow
    pG=exp(pG)
    pG=pG/matrix(nrow=nrow(pG),ncol=ncol(pG),data=colSums(pG),byrow=T)
    return(pG)
}
