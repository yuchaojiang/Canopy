canopy.cluster=function(R, X, num_cluster, num_run, Mu.init=NULL, Tau_Kplus1=NULL){
        ...
        for(run in 1:num_run){
            ...
            if(K==1){
                ...
                if (run==1 & (!is.null(Mu.init))){
                    ...
                    VAF.pheat=pheatmap(VAF,cluster_rows = T,cluster_cols = F,kmeans_k=K,silent=T, clustering_distance_rows = "euclidean")
                    Mu=pmax(VAF.pheat$kmeans$centers,0.001)
                } else{
                    VAF.pheat=pheatmap(VAF,cluster_rows = T,cluster_cols = F,kmeans_k=K,silent=T, clustering_distance_rows = "correlation")
                    Mu=pmax(VAF.pheat$kmeans$centers,0.001)
                }
            }
            diff=1
    ...
    mut_cluster=pGrank
    return(list(bic_output=bic_output,Mu=Mu,Tau=Tau,mut_cluster=mut_cluster))
}
