...
\verb@Canopy@'s webpage is \href{https://github.com/yuchaojiang/Canopy}{\textcolor{blue}{here}}. 
A demo code for \verb@Canopy@ can be found \href{https://github.com/yuchaojiang/Canopy/blob/master/demo_code/Canopy_demo.R}{\textcolor{blue}{here}}.
Script for dataset from the MDA231 study is attached below with step-by-step decomposition and explanation. Online Q\&A forum for
...
@
The most likely tree is shown below -- if you let the chain run long enough, there
should be only one tree configuration. The code for this toy dataset analysis 
